...
func _on_Area2D_body_entered(body):
	...
	if body.name != "rigid" and body != collided_with:
		...
		var speed = (collided_with.linear_velocity.y / 50) * motion_factor
		print(String(collided_with.linear_velocity.y) + " downForce")
		print(String(speed) + " result")
